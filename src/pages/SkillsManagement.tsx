...
export function SkillsManagement() {
  ...
  React.useEffect(() => {
    if (!token) return;
    api.mySkills(token).then((data) => setSkills(data as SkillItem[])).catch(() => setSkills([]));
  }, [token]);
  ...
  const addSkill = () => {
    ...
    if (token) {
      api
        .addSkill({ name: newSkill.name, icon: newSkill.icon, level: newSkill.level }, token)
        .then((created) => setSkills((prev) => [...prev, created as SkillItem]))
        .catch(() => undefined);
    } else {
  ...
  const removeSkill = (id: string) => {
    ...
    if (token) {
      api.removeSkill(id, token).catch(() => undefined);
    }
  };
  return (
    <div className="flex flex-col h-full bg-background relative">
      {/* Header */}
      <div className="bg-surface px-4 py-4 flex-shrink-0 border-b border-border">
